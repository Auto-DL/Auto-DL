...
def local_deploy(request):
    ...
    try:
        ...
        logging.info("Cloned Flask App to be deployed")
        ...
        logging.error("Unable to find the Project")
        raise

    except CloneGenerationFailed:
        logging.error("Unable to clone the Flask App")
        raise

    except AppUpsertionFailed:
        logging.error("Unable to modify the Flask App")
        raise

    except AppDownloadFailed:
        logging.error("Unable to download the Flask App")
        raise

    else:
        logging.info("Local Deployment successful")
        return response
...
def cloud_deploy(request):
    ...
    try:
        ...
        if current_chunk == total_chunks:
            logging.info("Pickle file uploaded successfully")
            model_categories = request.data.get("model_categories")
            ...
            deployment.clone_flask_app()
            logging.debug("Cloned Flask App to be deployed")
        ...
        logging.error("Unable to append Pickle file contents")
        remove_dir(pkl_dir)
        raise
        ...
        logging.error("Unable to copy pickle file into deployment directory")
        remove_dir(deployment_dir)
        remove_dir(pkl_dir)
        raise
        ...
        logging.info("Cloud Deployment Successful")
...
def hybrid_deploy(request):
    ...
    try:
        ...
        if current_chunk == total_chunks:
            logging.info("Pickle file upload successful")
            model_categories = request.data.get("model_categories")
            model_download_type = request.data.get("model_download_type")
        ...
        logging.error("Unable to copy file into deployment directory")
        remove_dir(deployment_dir)
        remove_dir(pkl_dir)
        raise
        ...
        logging.info("Hybrid Deployment successful")
        return response
